...
func compilePkg(args []string) error {
	...
	var outPath, outFactsPath, cgoExportHPath, genOutDir string
	...
	fs.StringVar(&genOutDir, "genoutdir", "", "The directory to store Go files generated by compilepkg and needed for export to other tools.")
	...
	if genOutDir == "" {
		return fmt.Errorf("-genoutdir is a required paramater when building cgo code (this is a rules_go internal error)")
	}

	if err := os.Mkdir(genOutDir, 0755); err != nil {
		return err
	}
...
func compileArchive(
	...
	if cgoEnabled && haveCgo {
		...
		// compile .S files.
		var srcDir string
		srcDir, goSrcs, objFiles, err = cgo2(goenv, goSrcs, cgoSrcs, cSrcs, cxxSrcs, objcSrcs, objcxxSrcs, nil, hSrcs, packagePath, packageName, cc, cppFlags, cFlags, cxxFlags, objcFlags, objcxxFlags, ldFlags, cgoExportHPath, genOutDir)
		if err != nil {
			return err
		}
